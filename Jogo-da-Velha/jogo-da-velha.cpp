...
// Área de definição de cabeçalho de funçào
void menuInicial();
...
static void iniciaTabuleiro(char tabuleiro[3][3]) {
	...
	// Navega por cada posiçào do tabuleiro e coloca o símbolo de '-'
	int linha, coluna;
...
static void jogo(string nomeDoJogadorUm, string nomeDoJogadorDois, int pontuacaoJogadorUm, int pontuacaoJogadorDois) {
	...
	while (rodada < 9 && estadoDeJogo == 1) {
		...
		// Exibe qual número corresponde a qual posiçào
		exibeInstrucoes();
		...
		while (posicionouJogada == false) {
			...
			// Lê a jogada
			cout << "\n\n" << nomeDoJogadorAtual << " digite uma posição conforme o mapa acima: ";
			cin >> posicaoJogada;
